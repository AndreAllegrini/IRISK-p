...
- Only code to plot results (e.g. flowchart not included).

- Summary-level data to reproduce the plots are included in `output` folder.
...
levels(df$PRS) <- c("PC1","Polygenic-P","ADHD","AN","ANX" ,        "AUT",
...
right_label$Alpha <- ifelse(right_label$`direct test`=="*", 1,
                           ifelse(right_label$`indirect test`=="*", 1, .3)) #fading non significant assoc
...
p1 <- ggplot(data=right_label, aes(y=PRS, x=Beta, xmin=Beta-(1.96*SE), xmax=Beta+(1.96*SE),color=Role,group =Role,shape = Role))+ 
  ...
  facet_grid(PRS ~ Outcome, scales= "free", space="free") +
    geom_text(data = right_label,aes(label=`direct test`),hjust=0.5, vjust=0.5, color = 'black',
              position=position_dodge(width=.6), show.legend = F,size = 8) +
  
  geom_text(data = right_label,aes(label=`indirect test`),hjust=0.5, vjust=0.5, color = 'black', 
            position=position_dodge(width=.6), show.legend = F,size = 8) +
...
levels(datf$df2.PRS) <- c("PC1","Polygenic-P","ADHD","AN","ANX" ,"AUT",
"BIP","DEP","PTSD","SCZ","Insomnia","Neuroticism"
, "CPAIN","Hair Colour")
...
levels(datf$univ.PRS) <- c("PC1","Polygenic-P","ADHD","AN","ANX" ,        "AUT",
"BIP","DEP","PTSD","SCZ","Insomnia","Neuroticism"
, "CPAIN","Hair Colour")
...
p <- ggplot(data) +
  ...
  #segments 
  geom_segment( aes(x=reorder_within2(univ.PRS, perc_dec,univ.Outcome), xend=reorder_within2(univ.PRS, perc_dec,univ.Outcome), y=univ.stdBeta , yend=cond.StdBeta
                    #, color="black"
                    , alpha = 1)) +
...
p <- ggplot(data=right_label, aes(y=PRS, x=Beta, xmin=Beta-(1.96*SE), xmax=Beta+(1.96*SE),color=Role,group =Role, shape = Role))+ 
  ...
  geom_errorbarh(data=right_label,height=.1, position=position_dodge(width=.6),alpha=right_label$Alpha, size = 1) +
   ...
   geom_rect(data = subset(df, PRS %in% c("Polygenic-P","DEP","AUT") & Outcome %in% c("P" )), 
...
p <- power_plot(power, save =F,out=nameOutDir) +  theme_bw() + theme(legend.position = "none", 

 axis.text.x = element_text(face="bold", size=16, angle = 90),
        axis.text.y = element_text(face="bold", size=16,angle=0),
        text = element_text(size = 24),
 strip.background = element_blank(),
    strip.placement = "outside") + xlab("father genetic effect")
...
p <- ggplot(datf,aes(x=df2.stdBeta, y=df.StdBeta, color="gray")) +
  ...
  geom_text_repel(data=wrong_label,aes(label = df2.PRS,color=df2.PRS),size = 5,alpha=.4
                 # ,max.overlaps=Inf
                  ) + 
  geom_text_repel(data=right_label,aes(label = df2.PRS,color=df2.PRS),size = 5,
                #  ,max.overlaps=Inf, 
                  fontface = "bold") +
...
p <- ggplot(datf, aes(x=lambda, y=beta, color="gray")) +
  geom_point() + 
   geom_errorbar(aes(ymin = beta-se, ymax = beta+se)) + 
   geom_abline(slope = coef(fitall)[[1]], 
              colour = "red", linetype =2, alpha=.5) + 
  geom_text_repel(aes(label = item,color=item),size = 4) + 
  #  scale_x_continuous(limits=c(.7,1.3)
...
slopEst <- data.frame(beta=rbind(coef(summary(lm(beta ~ 0 + lambda, datf[datf$role == "child",])))[[1]],
...
p <- ggplot(datf, aes(x=lambda, y=beta, color="gray")) +
  geom_point() + 
   ...
   geom_abline(data=slopEst,aes(slope=beta, 
              intercept = 0), colour = "black", linetype =2, alpha=.5) + 
  geom_text_repel(aes(label = item,color=item),size = 4) + 
    xlab("lambda") + 
  ylab("beta" ) +
...
ggsave(p, file=paste0("../figures/Figure_S9.png"), width = 8, height=8, dpi=750)
...
ggsave(p, file=paste0("../figures/Figure_S11.png"),  width = 8, height=8, dpi=750)
